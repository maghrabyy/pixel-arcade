...
export const RockPaperScissors = () => {
  ...
  const [playerSelection, setPlayerSelection] = useState<GameSelection | null>(
    null
  );
  const [pcSelection, setPCSelection] = useState<GameSelection | null>(null);
  const [roundCondition, setRoundCondition] = useState<RoundCondition | null>(
    null
  );
  ...
  return (
    <div className="main-menu bg-gradient-to-r from-[#373438] to-[#1e1d1f] 5 w-full h-[220px] p-1">
      {gamestate === GameState.mainMenu ? (
        <GameMainMenu setGamestate={setGamestate} />
      ) : (
        <div className="gameplay h-[calc(100%-32px)]">
          <div className="game-header flex justify-between px-2">
          ...
          {gamestate === GameState.playing ? (
            pcTurn ? (
              <div className="pc-turn flex justify-center items-center h-full gap-2 relative">
                <motion.img
              ...
              <div className="player-turn flex flex-wrap sm:gap-2 justify-center items-center h-full">
                <div
                  onClick={() => playerSelectionHandler(GameSelection.rock)}
                ...
                >
                  <img src={rockImg} className="w-20 sm:w-36" alt="rock" />
                </div>
                <div
                  onClick={() => playerSelectionHandler(GameSelection.paper)}
                  className="round-selection cursor-pointer p-1 duration-300 ease-out rounded-md hover:bg-black hover:bg-opacity-20 hover:scale-105"
                >
                  <img src={paperImg} className="w-20 sm:w-36" alt="paper" />
                </div>
                <div
                  onClick={() => playerSelectionHandler(GameSelection.scissor)}
                  className="round-selection cursor-pointer p-1 duration-300 ease-out rounded-md hover:bg-black hover:bg-opacity-20 hover:scale-105"
                >
                  <img
                    src={scissorImg}
            ...
            <div className="game-over flex flex-col justify-center items-center gap-2">
              ...
              <Button width={200} onClick={replayGame}>
                replay
              </Button>
              <Button width={200} onClick={returnToMainMenu}>
                Main Menu
              </Button>
            </div>
          )}
